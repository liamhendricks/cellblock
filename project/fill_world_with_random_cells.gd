...
var registry_resource_path : String = "res://addons/cellblock/resources/example_registry_1.tres"
...
func _run() -> void:
	...
	ResourceSaver.save(cell_registry, registry_resource_path, ResourceSaver.FLAG_BUNDLE_RESOURCES)

func create_cell_scenes_in_grid(cell_registry : CellRegistry):
	cell_registry.cell_data = []
	...
	for x in range(-floor((cell_registry.grid_size.x / cell_registry.cell_size) / 2), floor((cell_registry.grid_size.x / cell_registry.cell_size) / 2) + 1):
		for z in range(-floor((cell_registry.grid_size.z / cell_registry.cell_size) / 2), floor((cell_registry.grid_size.z / cell_registry.cell_size) / 2) + 1):
			...
			cell_registry.cell_data.append(cell_data)
...
func _randomize_block(bblock : StaticBody3D) -> void:
	bblock.name = "block"
	var scale = randi_range(1, high_end_scale)
	bblock.global_scale(Vector3(scale, scale, scale))

func _randomize_ball(bball : RigidBody3D) -> void:
	bball.name = "ball"
	var x = randi_range(4, 8)
	var z = randi_range(5, 6)
	bball.global_position.x = x
	bball.global_position.z = z
	bball.global_position.y = 2
