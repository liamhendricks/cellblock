...
func work_all_cells(origin_object : Node3D):
	print("len: ", len(cell_registry.cells))
...
func _work_cell(origin_object : Node3D, _coords : Vector3i) -> float:
	var in_range : bool = false
	var cd : CellData = cell_registry.cells[_coords]
	var cell_location = CellManager.cell_to_world_space(cd.coordinates, cell_registry.cell_size)
	var current_cell_location = CellManager.cell_to_world_space(current_cell_coords, cell_registry.cell_size)
	var dist = cell_location.distance_to(origin_object.global_transform.origin)
	var dist_to_current = current_cell_location.distance_to(origin_object.global_transform.origin)
	if dist < cd.distance_to_origin:
		in_range = true
	...
	if in_range:
		if cd.coordinates not in cell_loader.active_cells:
			print("dist: ", dist, cd.coordinates, cd.distance_to_origin)
			cell_loader.add(cd)
	else:
